...
export async function POST(req: Request) {
    ...
    // We use Payload Local API (instead of POSTing to /api/task-attachments) because on some
    ...
    const relatedTask = (await payload.findByID({
        collection: "tasks",
        id: taskID,
        depth: 0,
        overrideAccess: true,
    ...
    const careGroupID =
        ...
        typeof relatedTask?.careGroup === "number"
            ...
            : (relatedTask?.careGroup as { id?: string | number } | undefined)
                  ?.id;
    ...
    const taskType = relatedTask?.type;
